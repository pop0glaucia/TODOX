...
foreach ($Teste in $MbxTeste) {
    ...
    if (!(Get-ExchangeServer | Get-MessageTrackingLog -Start (get-date).AddDays(-[int]$Date) -Sender "$($PrimarySmtpAddress)" -ResultSize Unlimited | Select-Object `
    ...
    {
        Get-ExchangeServer | Get-MessageTrackingLog -Start (get-date).AddDays(-[int]$Date) -Sender "$($PrimarySmtpAddress)" -ResultSize Unlimited | Select-Object `
            @{N='TimeStamp'; E={$_.TimeStamp}} ,      
            @{N='Sender'; E={$_.Sender}},            
            @{N='Recipients'; E={$_.Recipients}},   
            @{N='MessageSubject'; E={$_.MessageSubject}},
    ...
    if (!(Get-ExchangeServer | Get-MessageTrackingLog -Start (get-date).AddDays(-[int]$Date) -Sender "$($PrimarySmtpAddress)" -ResultSize Unlimited | Select-Object * | Get-MessageClientProtocol ))
    {
        $PrimarySmtpAddress | Select-Object `
            @{N='ProgressColeta'; E={$("$([String]$Contador++)" + '..de..' + "$Total")}},
            @{N='TimeStamp'; E={''}} ,      
    ...
    {
        Get-ExchangeServer | Get-MessageTrackingLog -Start (get-date).AddDays(-[int]$Date) -Sender "$($PrimarySmtpAddress)" -ResultSize Unlimited | Select-Object * | Get-MessageClientProtocol  | Select-Object @{N='ProgressColeta'; E={$("$([int]$Contador++)" + '..de..' + "$Total")}},*,@{N='Note'; E={'A caixa ENVIOU mensagens utilizando uma conexao IMAP, POP or SMTP'}},@{N='MailboxLogon'; E={$($LastLogonTime)}} | ft -AutoSize ProgressColeta,TimeStamp,Sender,Recipients,MessageSubject,ClientType,Note,MailboxLogon
    }
}
...
#Get-ExchangeServer | Get-MessageTrackingLog -Start "9/1/2022 0:01:00 AM" -Sender joao.santos@servicein.tec.br -ResultSize Unlimited  | Get-MessageClientType | ? {($_.EventId -notlike 'SUBMITDEFER')} | ft -AutoSize
#Get-ExchangeServer | Get-MessageTrackingLog -Start "9/1/2022 0:01:00 AM" -Sender "NO-REPLY@SERVICEIN.TEC.BR" -ResultSize Unlimited  | Get-MessageClientType | ? {($_.MessageSubject -like $null)} | ft -AutoSize
...
foreach ($Mailbox in $Lista) {
    ...
    {
        $Arr2 = Get-ExchangeServer | Get-MessageTrackingLog -Start (get-date).AddDays(-[int]$Date) -Sender "$($PrimarySmtpAddress)" -ResultSize Unlimited | Select-Object `
            @{N='TimeStamp'; E={$_.TimeStamp}} ,      
            @{N='Sender'; E={$_.Sender}},            
            @{N='Recipients'; E={$_.Recipients}},   
            @{N='MessageSubject'; E={$_.MessageSubject}},
    ...
    {
        $Arr4 = Get-ExchangeServer | Get-MessageTrackingLog -Start (get-date).AddDays(-[int]$Date) -Sender "$($PrimarySmtpAddress)" -ResultSize Unlimited | Select-Object *   | Get-MessageClientProtocol  | Select-Object `
        @{N='ProgressColeta'; E={$("$([int]$Contador++)" + '..de..' + "$Total")}},
        *,
